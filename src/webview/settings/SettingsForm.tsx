...
export function SettingsForm({ data }: { data: SettingsData }) {
    ...
    const handleMessage = useCallback((e: MessageEvent) => {
        ...
        if (msg.command === 'settingsUpdateForm') {
            if (msg.userName !== undefined) setUserName(msg.userName);
            if (msg.userEmail !== undefined) setUserEmail(msg.userEmail);
            if (msg.remotes) setRemotes(msg.remotes);
        }
    }, []);
        ...
        <div style={{ padding: '20px 24px', maxWidth: 520, fontFamily: 'var(--vscode-font-family)', fontSize: '13px', color: 'var(--vscode-foreground)' }}>
            ...
            <div style={{ marginBottom: 14 }}>
                ...
                <input
                    type="email"
                    value={userEmail}
                    onChange={(e) => { setUserEmail(e.target.value); setEmailChanged(true); }}
                    onBlur={applyUserEmail}
                    onKeyDown={(e) => { if (e.key === 'Enter') applyUserEmail(); }}
