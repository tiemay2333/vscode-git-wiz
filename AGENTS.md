...
> 3. **手动归档流程**：任务必须保持在 `in_progress` 状态，直到用户确认代码已成功提交。归档操作必须由用户手动通过 `finish-work` 命令执行。
> 4. **单次授权原则**：上述`CAUTION`内容的用户的同意或者授权，仅代表对当次操作的授权，不代表在本次会话中的持续同意。每次执行受限操作前都必须单独获取用户的明确授权。
